...
export const firebaseClient = {
  auth: {
    ...
    redirectToLogin: async () => {
      ...
      try {
        ...
        if (ALLOWED_ADMIN_EMAILS.includes(user.email)) {
          ...
          // Return true so the calling component can update its state
          return true
        } else {
          await signOut(auth)
    ...
    logout: async () => {
      try {
        ...
        // No redirect — let the component handle state update
      } catch (error) {
  ...
  entities: {
    ...
    SiteSettings: {
      get: async () => {
        try {
          const docRef = doc(db, 'site_config', 'site_settings')
          const docSnap = await getDoc(docRef)
          ...
          if (docSnap.exists()) {
            ...
            return { loginPassword: 'saugat@123' }
          }
        } catch (error) {
...
function normalizeDoc(docSnap) {
  ...
  return {
    ...
    ...data,
    created_date: data.created_date?.toDate?.() || (data.created_date instanceof Date ? data.created_date : new Date()),
    published_date: data.published_date?.toDate?.() || (data.published_date instanceof Date ? data.published_date : new Date()),
    updated_date: data.updated_date?.toDate?.() || (data.updated_date instanceof Date ? data.updated_date : new Date()),
  }
}
...
async function getCollection(collectionName, orderByField = 'created_date') {
  try {
    ...
    try {
      ...
      console.warn(`OrderBy '${field}' failed for ${collectionName}, fetching without order:`, orderError.message)
      querySnapshot = await getDocs(collection(db, collectionName))
    }

    const results = querySnapshot.docs.map(normalizeDoc)
...
async function getFilteredCollection(collectionName, filters, orderByField = 'created_date', limit = null) {
  try {
    ...
    let results = querySnapshot.docs.map(normalizeDoc)
